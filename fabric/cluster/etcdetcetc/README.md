...
write left by a crashed Job. Its digest-pinned purpose-built image copies only
`etcdctl` 3.6.13 from the digest-pinned official etcd image into the pinned
BusyBox userspace, so the runtime test has no router-asset dependency after the
image pull. Its label-scoped NetworkPolicy permits only the three physical root
`/32`s on TCP/2379; it has no ServiceAccount token or DNS egress. A successful
run emits one non-secret `FABRIC_ETCD_SMOKE=pass` marker, while failed
